...
function getComponent(info) {
	...
	switch(info.type) {
		case 'org':
			cmp = getOrganizationCmp(info);
			break;
		case 'uni':
			cmp = getUnitCmp(info);
			break;
		case 'pro':
			cmp = getProjectCmp(info);
			break;
		case 'emp':
			cmp = getEmployeeCmp(info);
			break;			
	}
	return cmp;
}
...
function getOrganizationCmp(info) {
